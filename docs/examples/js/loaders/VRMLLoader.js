...
THREE.VRMLLoader.prototype = {
	...
	parse: function ( data ) {
		...
		function parseV2( lines, scene ) {
			...
			function parseNode( data, parent ) {
              ...
              for ( i = 0, il = data.coordIndex.length; i < il; i ++ ) {
								...
								var indexes2 = data.texCoordIndex[ i ];

								// VRML support multipoint indexed face sets (more than 3 vertices). You must calculate the composing triangles here
								...
								while ( indexes.length >= 3 && skip < ( indexes.length - 2 ) ) {
									...
									var j1 = indexes2[ 0 ];
									var j2 = indexes2[ skip + ( data.ccw ? 1 : 2 ) ];
									var j3 = indexes2[ skip + ( data.ccw ? 2 : 1 ) ];
									...
									if ( uvs.length > 0 ) {
                    uv.fromArray( uvs, j1 * 2 );
										newUvs.push( uv.x, uv.y );
										uv.fromArray( uvs, j2 * 2 );
										newUvs.push( uv.x, uv.y );
										uv.fromArray( uvs, j3 * 2 );
										newUvs.push( uv.x, uv.y );
									}
          ...
          for ( var i = 0; i < data.children.length; i ++ ) {
              ...
              useMaterial = true;
							...
							if ( textureName ) {
								...
								material.name = textureName[ 1 ];

								material.map = textureLoader.load( texturePath + textureName[ 1 ] );
            ...
            if ( child.nodeType === 'TextureTransform' ) {
							if(material.map && material.map.repeat) {
                material.map.repeat.set(child.scale.x,child.scale.y);
              }
						}
      ...
      var theTree = getTree( lines );

			parseNode( theTree, scene );
